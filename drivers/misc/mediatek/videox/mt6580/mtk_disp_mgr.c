	...
	if(DISP_SESSION_TYPE(session_id) == DISP_SESSION_PRIMARY)
	{
		...
		{	
			...
			if(session_input.config[i].layer_enable)
			{
				...
				{
					dst_mva = session_input.config[i].src_phy_addr;
				}
				else
				{
					disp_sync_query_buf_info(session_id, layer_id, (unsigned int)session_input.config[i].next_buff_idx, &dst_mva, &dst_size);
				...
				if(dst_mva == 0)
				{
					...
					disp_input_config *input = &session_input.config[i];  //FIXME: idle pointer?
				}
				DISPPR_FENCE("S+/L%d/e%d/id%d/%dx%d(%d,%d)(%d,%d)/%s/%d/pa%p/mva0x%08x\n", 
			...
			{
				DISPPR_FENCE("S+/L%d/e%d/id%d\n", session_input.config[i].layer_id, session_input.config[i].layer_enable, session_input.config[i].next_buff_idx);
			}
			
			disp_sync_put_cached_layer_info(session_id, layer_id, &session_input.config[i], dst_mva);
			_sync_convert_fb_layer_to_disp_input(session_input.session_id, &(session_input.config[i]),&primary_input[layer_id], dst_mva);

			if(session_input.config[i].layer_enable)
			{
				// OVL addr is not the start address of buffer, which is calculated by pitch and ROI.
				mva_offset = primary_input[layer_id].src_x* (primary_input[layer_id].src_pitch/session_input.config[i].src_pitch) + primary_input[layer_id].src_y * primary_input[layer_id].src_pitch;
				mtkfb_update_buf_info(session_input.session_id, session_input.config[i].layer_id, session_input.config[i].next_buff_idx, mva_offset);
			}

			primary_input[layer_id].dirty = 1;
			if(session_info)
			{
				dprec_submit(&session_info->event_setinput, session_input.config[i].next_buff_idx, dst_mva);
			}
		}
	...
	else if(DISP_SESSION_TYPE(session_id) == DISP_SESSION_EXTERNAL)
	{
        ...
        for (i = 0; i < session_input.config_layer_num; ++i)
		{
			...
			if(session_input.config[i].layer_enable)
			{
				...
				DISPPR_FENCE("S+/L%d/e%d/id%d/%dx%d(%d,%d)(%d,%d)/%s/%d/0x%08x/mva0x%08x\n", 
					session_input.config[i].layer_id, 
					session_input.config[i].layer_enable, 
					session_input.config[i].next_buff_idx,
					session_input.config[i].src_width, 
            ...
            disp_sync_put_cached_layer_info(session_id, layer_id, &session_input.config[i], dst_mva);
			_sync_convert_fb_layer_to_disp_input(session_input.session_id, &(session_input.config[i]),&extd_input[layer_id], dst_mva);

			extd_input[layer_id].dirty = 1;
            
			if(session_input.config[i].layer_enable)
    		{
    		    mva_offset = extd_input[layer_id].src_x* (extd_input[layer_id].src_pitch/session_input.config[i].src_pitch) + extd_input[layer_id].src_y * extd_input[layer_id].src_pitch;
    			mtkfb_update_buf_info(session_input.session_id, session_input.config[i].layer_id,
			                                session_input.config[i].next_buff_idx, mva_offset);
            }

            if(session_info)
			{
				dprec_submit(&session_info->event_setinput, session_input.config[i].next_buff_idx, 
				        (session_input.config_layer_num<<28)|(session_input.config[i].layer_id<<24)|(session_input.config[i].src_fmt<<12)|session_input.config[i].layer_enable);
			}
		}
		
		extd_hdmi_config_input_multiple(&extd_input, session_input.config[0].next_buff_idx);
#endif
	}
	else if(DISP_SESSION_TYPE(session_id) == DISP_SESSION_MEMORY)
	{
        ...
        for (i = 0; i < session_input.config_layer_num; i++)
		{
			...
			if(session_input.config[i].layer_enable )
			{
                ...
                DISPPR_FENCE("S+/L%d/e%d/id%d/%dx%d(%d,%d)(%d,%d)/%s/%d/pa%p/mva0x%08x/t%d\n", 
					session_input.config[i].layer_id, 
					session_input.config[i].layer_enable, 
					session_input.config[i].next_buff_idx,
					session_input.config[i].src_width, 
			...
			_sync_convert_fb_layer_to_ovl_struct(session_input.session_id, &(session_input.config[i]),&ovl2mem_in_cached_config[layer_id], dst_mva);
			///disp_sync_put_cached_layer_info(session_id, layer_id, &session_input.config[i], get_ovl2mem_ticket());
			mtkfb_update_buf_ticket(session_id, layer_id, session_input.config[i].next_buff_idx,get_ovl2mem_ticket());
			_sync_convert_fb_layer_to_disp_input(session_input.session_id, &(session_input.config[i]),&input_params[layer_id], dst_mva);
...
int _ioctl_set_output_buffer(unsigned long arg)
{
    ...
    else if(DISP_SESSION_TYPE(session_id) == DISP_SESSION_MEMORY)
    {
        ...
        {
            dst_mva = mtkfb_query_buf_mva(session_id, DDP_OUTPUT_LAYID  , (unsigned int)session_output.config.buff_idx);
        }

        mtkfb_update_buf_ticket(session_id, DDP_OUTPUT_LAYID, session_output.config.buff_idx, get_ovl2mem_ticket());
        _sync_convert_fb_layer_to_disp_output(session_output.session_id, &(session_output.config), &primary_output, dst_mva);
        primary_output.dirty = 1;

        DISPPR_FENCE("S+/%s%d/L%d/id%d/%dx%d(%d,%d)/%s/%d/%d/pa%p/mva0x%08x/t%d\n", 
					disp_session_mode_spy(session_id), DISP_SESSION_DEV(session_id), 
					4,
					session_output.config.buff_idx,
					session_output.config.width, 
