...
static int _convert_disp_input_to_ovl(OVL_CONFIG_STRUCT *dst, primary_disp_input_config* src)
{
	if(src && dst)
	{
		...
		{
			DISPERR("unknow source = %d", src->buff_source);
...
static int _fence_release_worker_thread(void *data)
{
	...
	while(1)
	{
        ...
        {
            continue;
            // HWC not enabled, pan_display, dose not use fence, so just skip
        }
		...
		for(i=0;i<PRIMARY_DISPLAY_SESSION_LAYER_COUNT;i++)
		{
			...
			{
				mtkfb_release_layer_fence(session_id, (OVL_LAYER_NUM-1));
			}
			else
			{
				...
				if((fence_idx<0))
				{
					...
					{
                        // DISPPR_ERROR("find fence idx for layer %d,addr 0x%08x fail, return -2\n", i, addr);
					}
					else
					{
...
{
	printk("primary_display_init begin\n");
	DISP_STATUS ret = DISP_STATUS_OK;
	DISP_MODULE_ENUM dst_module = 0;
...
int primary_display_capture_framebuffer_ovl(unsigned long pbuf, unsigned int format)
{
	...
	if(m4uClient == NULL)
	{
		DISPCHECK("primary capture:Fail to alloc  m4uClient %p\n",m4uClient);
        ret = -1;
        goto out;
	}
...
int fbconfig_get_esd_check_test(UINT32 dsi_id,UINT32 cmd,UINT8*buffer,UINT32 num)
{
	...
	dpmgr_path_reset(pgc->dpmgr_handle, CMDQ_DISABLE);
	/*FIXME: panel master not ready on K2
