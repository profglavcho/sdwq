...
static int mt_eint_is_debounce_en(unsigned int cur_eint_num)
{
	...
	if (EINT_AP_MAXNUMBER > MAX_HW_DEBOUNCE_CNT) {
...
static int __init mt_eint_init(void)
{
    ...
    if(node) {
        ...
        printk("[EINT] can't find compatible node\n");
        return -1;
    }
    ...
    printk("EINT_IRQ_BASE = %d, count = %d, nr_irq = %d\n", EINT_IRQ_BASE, EINT_MAX_CHANNEL, nr_irqs);
