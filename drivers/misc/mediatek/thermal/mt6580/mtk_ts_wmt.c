...
#define MID_WIFI_POWER 100
#define WIFI_POLLING_FACTOR_UP 1 //assume WiFi polling interval is 50ms * 20

// Interal usage
static int g_curr_temp = 0;
static int g_prev_temp = 0;
static int cur_fp_factor = 1;
static int wifi_target_tj = 46000;
static int wifi_target_offset = 5000;
static int g_trigger_from_cpu = 0;
...
static int target_tj_source = 2;                        // 0 from wmt , 1 from cpu, 2 is null 
static int wmt_wifi_target_tj = 46000;
static int wmt_wifi_target_offset = 5000;
static int tj_stable_range = 1000;
static int resource_allocator_policy = 0;               //0: wifi last (default) 1: wifi first 2: no throttle wifi 3: independent
static int min_wifi_tput_ratio = 40;
static int max_wifi_tput_ratio = 200;
static int min_wifi_tput = 1000;
static int tt_wifi_high = 100 * WIFI_POLLING_FACTOR_UP; // inital value: assume 1 degreeC for temp. <=> 1 unit for total_power(0~100)
static int tt_wifi_low = 100 * WIFI_POLLING_FACTOR_UP;
static int tp_wifi_rise = 100 * WIFI_POLLING_FACTOR_UP; 
static int tp_wifi_fall = 100 * WIFI_POLLING_FACTOR_UP;
...
{  
    unsigned long limit_tput;   

    limit_tput = (((cur_tput << 10) * limit_ratio) / MID_WIFI_POWER) >> 10;    
    limit_tput = (limit_tput > 1000) ? limit_tput : 1000;

    wmt_tm_dprintk("%s Curr Tput %d ratio %d limit_tput %d",__func__,tx_throughput,limit_ratio,limit_tput);
...
{	
    ...
    if (cl_dev_adp_cpu_state_active == 1) {     
        ...
        tt_wifi = tt_wifi * WIFI_POLLING_FACTOR_UP / cur_fp_factor;             
        tp_wifi = tp_wifi * WIFI_POLLING_FACTOR_UP / cur_fp_factor;         
#else    
        tt_wifi = tt_wifi * WIFI_POLLING_FACTOR_UP;             
        tp_wifi = tp_wifi * WIFI_POLLING_FACTOR_UP;         
#endif 
        ...
        if(((curr_temp > WIFI_TARGET_TJ_HIGH) && (curr_temp > prev_temp)) ||  
            ((curr_temp <= WIFI_TARGET_TJ_HIGH) && (curr_temp >= WIFI_TARGET_TJ_LOW) && (curr_temp >= (prev_temp + (tj_stable_range * 2))))){
                total_power += (tt_wifi / tt_wifi_high + tp_wifi / tp_wifi_rise);            
        } else if (((curr_temp > WIFI_TARGET_TJ_HIGH) && (curr_temp <= prev_temp)) || 
            ((curr_temp <= WIFI_TARGET_TJ_HIGH) && (curr_temp >= WIFI_TARGET_TJ_LOW) && (curr_temp <= (prev_temp - (tj_stable_range * 2))))){
                total_power += (tt_wifi / tt_wifi_high + tp_wifi / tp_wifi_fall);            
        } else if ((curr_temp < WIFI_TARGET_TJ_LOW) && (curr_temp > prev_temp)) {
                total_power += (tt_wifi / tt_wifi_low + tp_wifi / tp_wifi_rise);            
        } else if((curr_temp < WIFI_TARGET_TJ_LOW) && (curr_temp <= prev_temp)) {
                total_power += (tt_wifi / tt_wifi_low + tp_wifi / tp_wifi_fall);            
        }            
        
        total_power = (total_power > min_wifi_tput_ratio ) ? total_power : min_wifi_tput_ratio;            
        total_power = (total_power < max_wifi_tput_ratio) ? total_power : max_wifi_tput_ratio;
        wmt_tm_dprintk("%s Wifi TJ %d Tp %d, Tc %d, total_power %d tput %d\n", __func__, wifi_target_tj, prev_temp, curr_temp, total_power,tx_throughput);               
        set_adaptive_wifi_tput_limit(tx_throughput,total_power);        
    
    } else {       
        if (triggered) {           
            triggered = 0;          
            wmt_tm_dprintk("%s Tp %d Tc %d Pt %d\n", __func__, prev_temp, curr_temp, tx_throughput); 
            wmt_send_signal(-1);                    
        }
    }   
...
static void resource_allocator_power_tput(int temp)
{

    wmt_tm_dprintk("%s tj_source %d trigger_from_cpu %d\n", __func__, target_tj_source,g_trigger_from_cpu);

    if(target_tj_source == 0) return;   // origina wmt throttle 
    
    if(target_tj_source == 1 && g_trigger_from_cpu == 1){
        wifi_target_tj = get_cpu_target_tj();
        wifi_target_offset = get_cpu_target_offset();
    } else {
    ...
    switch(resource_allocator_policy){
        ...
        case 3://independent 
            ...
            cl_dev_adp_cpu_state_active = (temp >= ((wifi_target_tj - wifi_target_offset) / 1000)) ? 1 : 0;
            adaptive_tput_ratio((1000*g_prev_temp), (1000*temp));
            break;
...
int wifi_in_soc_throttle_from_cpu(int state)
{
    wmt_tm_dprintk("%s state %d\n", __func__, state);
    g_trigger_from_cpu = state;
                            ...
                            unsigned long *pv)
{
    ...
    int temp = 0;
    int temp_ts1 = 0, temp_ts2 = 0, temp_ts3 = 0, temp_ts4 = 0;
    ...
    *pv = 0;
    ...
    //if(pg_wmt_tm) {
    {
        ...
        temp_ts1 = get_immediate_ts1_wrap();
        temp_ts2 = get_immediate_ts2_wrap();
        temp_ts3 = get_immediate_ts3_wrap();
        //temp_ts5 = get_immediate_ts5_wrap();

        //temp_abb = get_immediate_abb_temp_wrap();   
        temp_cpu = get_immediate_cpu_wrap();
        temp_gpu = get_immediate_gpu_wrap();
        temp_soc = get_immediate_soc_wrap();

        wmt_tm_dprintk("%s TS1 %d TS2 %d TS3 %d \n",__func__,temp_ts1,temp_ts2,temp_ts3);
        //wmt_tm_dprintk("%s TABB %d TCPU %d TGPU %d TSOC %d\n",__func__,temp_abb,temp_cpu,temp_gpu,temp_soc);
        ...
        *pv =  temp*1000;
        resource_allocator_power_tput(temp);
...
ssize_t wmt_wifi_in_soc_write(struct file *filp, const char __user *buf, size_t len, loff_t *data)
{
	...
	ret = sscanf(tmp, "%d %d %d %d %d %d %d %d %d %d %d %d", &target_tj_source,
                                                             &wmt_wifi_target_tj,
	...
	wmt_tm_printk("%s ret %d %d %d %d %d %d %d %d %d %d %d %d", __func__, ret, target_tj_source,wmt_wifi_target_tj,
                    wmt_wifi_target_offset,tj_stable_range,resource_allocator_policy,min_wifi_tput_ratio,
                    max_wifi_tput_ratio,min_wifi_tput,tt_wifi_high,tt_wifi_low,tp_wifi_rise,tp_wifi_fall);
