...
#if RAINIER_PROJ
#define TS_CONFIGURE     TS_CON0    // depend on CPU design
...
#else
#define TS_CONFIGURE     TS_CON1    // depend on CPU design
#define TS_CONFIGURE_P   TS_CON1_P  // depend on CPU design
...
//mark for build pass    mt_gpufreq_thermal_protect((final_limit != 0x7FFFFFFF) ? final_limit : 0);
}
...
int tscpu_thermal_clock_on(void)
{
    ...
    //ret = enable_clock(MT_CG_PERI_THERM, "THERMAL"); [Rainier]Fix ME
...
int tscpu_thermal_clock_off(void)
{
    ...
    //ret = disable_clock(MT_CG_PERI_THERM, "THERMAL"); [Rainier]Fix ME
...
// Refactor
static int thermal_config_Bank(){
    int i,j = 0;
    int ret = -1;
    UINT32 temp = 0;
...
int get_immediate_cpu_wrap(void)
{
	...
	//curr_temp = CPU_TS_MCU2_T;
    curr_temp = max_temperature_in_bank(THERMAL_BANK0);
...
int get_immediate_gpu_wrap(void)
{
	...
	//curr_temp = GPU_TS_MCU1_T;
    curr_temp = max_temperature_in_bank(THERMAL_BANK1);
...
int get_immediate_soc_wrap(void)
{
	...
	//curr_temp = LTE_TS_MCU3_T;
    curr_temp = max_temperature_in_bank(THERMAL_BANK2);
...
extern int wifi_in_soc_throttle_from_cpu(int state);
...
static int P_adaptive(int total_power, unsigned int gpu_loading)
{
    ...
    else
    {
        ...
        int i = 0;
		unsigned int cur_gpu_freq = 0;//mark for build pass
//mark for build pass            unsigned int cur_gpu_freq = mt_gpufreq_get_cur_freq();
        //int cur_idx = 0;
...
{
//mark for build pass    		MAXIMUM_GPU_POWER =(int) mt_gpufreq_get_max_power();
//mark for build pass    		MINIMUM_GPU_POWER =(int) mt_gpufreq_get_min_power();
				...
				unsigned long state)
{
    ...
    if (active_adp_cooler == (int) (cdev->type[13] - '0'))
	{
        ...
        wifi_in_soc_throttle_from_cpu(state);
	return 0;
}
#endif
