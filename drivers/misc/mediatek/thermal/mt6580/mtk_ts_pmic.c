...
do {									\
    xlog_printk(ANDROID_LOG_ERROR, "Power/PMIC_Thermal", fmt, ##args); \
} while(0)
...
void mtktspmic_read_efuse(void)
{
    ...
    pmic_config_interface(0x010C, 0x1, 0x1, 12);

    //2. enable RD trig bit
    pmic_config_interface(0x0608, 0x1, 0x1, 0);

    //3. read thermal efuse
    //0x063A    160  175
    //0x063C    176  191
    //Thermal data from 161 to 184
    pmic_read_interface(0x063A, &efuse_val0, 0xFF, 0);
    pmic_read_interface(0x063C, &efuse_val1, 0xFF, 0);

    //4. polling RD busy bit
    do {
        pmic_read_interface(0x60C, &reg_val, 0x1, 0);
        mtktspmic_dbg("[mtktspmic_read_efuse] polling Reg[0x60C][0]=0x%x\n", reg_val);
    } while(reg_val == 1);

    //5. Disable efuse ctrl engine clock
    pmic_config_interface(0x010A, 0x1, 0x1, 12);

    mtktspmic_info("[mtktspmic_read_efuse] Reg(0x63a) = 0x%x, Reg(0x63c) = 0x%x\n", efuse_val0, efuse_val1);

    //6. parse thermal parameters
