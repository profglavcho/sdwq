...
{
    // check disp module
    if ((ddp_reg_pa_base[0] <= addr) &&
        (addr < ddp_reg_pa_base[DISP_REG_NUM-1]))
    {
        return dispsys_reg[0] + (addr - ddp_reg_pa_base[0]);
    }
...
static unsigned int color_va2pa(unsigned long addr)
{
    // check disp module
    if ((dispsys_reg[0] <= addr) &&
        (addr < dispsys_reg[DISP_REG_NUM-1]))
    {
        return ddp_reg_pa_base[0] + (addr - dispsys_reg[0]);
    }
    ...
    if ((g_tdshp_va <= addr) &&
        (addr < (g_tdshp_va + 0x1000)))
    {
        return TDSHP_PA_BASE + (addr - g_tdshp_va);
    }
}
...
static int _color_io(DISP_MODULE_ENUM module, int msg, unsigned long arg, void *cmdq)
{
    ...
    switch (msg) {
        ...
        case DISP_IOCTL_READ_REG:
        {
            ...
            value = (DISP_REG_GET(va)) & rParams.mask;

            COLOR_DBG("read pa:0x%x(va:0x%lx) = 0x%x (0x%x)\n", pa, va, value, rParams.mask);
        ...
        case DISP_IOCTL_WRITE_REG:
        {
            ...
            COLOR_DBG("write pa:0x%x(va:0x%lx) = 0x%x (0x%x)\n", pa, va, wParams.val, wParams.mask);
...
#else
        DISP_REG_MASK(NULL, DISP_COLOR_CFG_MAIN + offset, (1<<7), 0x00000080); //bypass all
#endif
...
#else
        DISP_REG_MASK(NULL, DISP_COLOR_CFG_MAIN + offset, (0<<7), 0x00000080); //resume all
#endif
