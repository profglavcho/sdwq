...
int ovl_start(DISP_MODULE_ENUM module, void *handle)
{
	...
	DISP_REG_SET(handle, idx_offset+DISP_REG_OVL_INTEN, 0x1E);
		...
		void *handle)
{
	...
	DDPDBG("ovl%d, layer=%d, source=%s, off(x=%d, y=%d), dst(%d, %d, %d, %d),pitch=%d,"
		"fmt=%s, addr=%lx, keyEn=%d, key=%d, aen=%d, alpha=%d,sur_aen=%d,sur_alpha=0x%x, constant_color=0x%x\n",
		idx,
		layer,
    ...
    DISP_REG_SET(handle, DISP_REG_OVL_L0_CLR+layer_offset, constant_color);
