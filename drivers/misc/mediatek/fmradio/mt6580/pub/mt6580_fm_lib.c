...
static fm_s32 mt6580_PowerUp(fm_u16 *chip_id, fm_u16 *device_id)
{
	...
	*device_id = tmp_reg;
	...
	if ((ret = mt6580_get_rom_version()) >= 0) {
...
*/
static fm_bool mt6580_Seek(fm_u16 min_freq, fm_u16 max_freq, fm_u16 *pFreq, fm_u16 seekdir,
			   fm_u16 space)
{
	fm_s32 ret = 0;
...
#if 0
static fm_bool mt6580_Scan(fm_u16 min_freq, fm_u16 max_freq, fm_u16 *pFreq, fm_u16 *pScanTBL,
			   fm_u16 *ScanTBLsize, fm_u16 scandir, fm_u16 space)
{
	...
	mt6580_full_cqi_get(min_freq, max_freq, space, 5);

	/* normal scan */
	if (FM_LOCK(cmd_buf_lock))
		return (-FM_ELOCK);
	pkt_size = mt6580_scan(cmd_buf, TX_BUF_SIZE, scandir, space, max_freq, min_freq);
	ret =
	...
	if (!ret && mt6580_res) {
		fm_memcpy(pScanTBL, mt6580_res->scan_result, sizeof(fm_u16) * FM_SCANTBL_SIZE);
		...
		*ScanTBLsize = tmp_scanTBLsize;
		...
		WCN_DBG(FM_ALT | CHIP, "mt6580_scan failed\n");
		return ret;
	}

	mt6580_set_bits(FM_MAIN_CTRL, 0x0000, 0xFFF0);	/* make sure tune/seek/scan/cqi bits = 0 */
	if ((temp & 0x0020) == 0) {
		mt6580_Mute(fm_false);
	}
...
static fm_s32 mt6580_CQI_Get(fm_s8 *buf, fm_s32 buf_len)
{
	fm_s32 ret = 0;
	fm_s32 i;
	fm_u16 pkt_size;
	struct mt6580_fm_cqi *pmt6580_cqi;
	...
	if (FM_LOCK(cmd_buf_lock))
		return (-FM_ELOCK);
	pkt_size = mt6580_cqi_get(cmd_buf, TX_BUF_SIZE);
	if (cqi_abort == fm_true) {
		...
		ret =
		    fm_cmd_tx(cmd_buf, pkt_size, FLAG_SCAN | FLAG_CQI_DONE, SW_RETRY_CNT,
			      SCAN_TIMEOUT, mt6580_get_read_result);
	}
	FM_UNLOCK(cmd_buf_lock);

	if (!ret && mt6580_res) {
		/* FIXEDME */
		pmt6580_cqi = (struct mt6580_fm_cqi *)mt6580_res->cqi;
		pcqi = (struct adapt_fm_cqi *)buf;

		for (i = 0; i < (sizeof(mt6580_res->cqi) / sizeof(struct mt6580_fm_cqi)); i++) {
			pcqi[i].ch = (pmt6580_cqi[i].ch * 10 / 2) + 6400;
			pcqi[i].rssi = (fm_s32) pmt6580_cqi[i].rssi;
		...
		WCN_DBG(FM_ALT | CHIP, "mt6580 get CQI failed:%d\n", ret);
	}
...
static fm_bool mt6580_Scan_50KHz(fm_u16 min_freq, fm_u16 max_freq, fm_u16 *pFreq,
				 ...
				 fm_u16 space)
{
	...
	for (i = 0; i < seg; i++) {
		...
		if (fm_false ==
		    mt6580_Scan(start_freq, end_freq, pFreq, scan_tbl, ScanTBLsize, scandir,
				space)) {
			return fm_false;
		}
		/* get channel count */
		...
		/* get cqi info */
		while (chl_cnt > 0) {
			ret = mt6580_CQI_Get(raw_buf, 16 * sizeof(struct adapt_fm_cqi));
			if (ret) {
				return ret;
			}
...
static fm_s32 mt6580_CQI_Get_50KHz(fm_s8 *buf, fm_s32 buf_len)
{
	fm_s32 ret = 0;
	fm_s32 i;
...
static fm_s32 mt6580_SeekStop(void)
{
	return fm_force_active_event(FLAG_SEEK_DONE);
}

static fm_s32 mt6580_ScanStop(void)
{
	cqi_abort = fm_true;
	scan_abort = fm_true;
...
static fm_s32 mt6580_desense_check(fm_u16 freq, fm_s32 rssi)
{
	if (mt6580_is_dese_chan(freq)) {
		...
		WCN_DBG(FM_DBG | CHIP, "desen_rssi %d th:%d\n", rssi,
			mt6580_fm_config.rx_cfg.desene_rssi_th);
	}
	return 0;
}
