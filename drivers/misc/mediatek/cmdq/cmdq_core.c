...
int cmdqCorePrintInstructionCountSeq(struct seq_file *m, void *v)
{
	...
	for (; numRec > 0; --numRec, --index) {
		...
		if ( (0 == record.instructionCount[CMDQ_INSTRUCTION_COUNT_EOC]) &&
			...
			seq_printf(m, "%4d, %5d, %02d, %4d", index, 'X', 'X', 0);
...
int cmdqCorePrintStatusSeq(struct seq_file *m, void *v)
{
	...
	if (gCmdqDumpFirstError.cmdqCount > 0){
		...
		seq_printf(m, "kernel time:[%5llu.%06lu], Pid: %d, Name: %s\n",
			saveTimeSec, rem_nsec / 1000, gCmdqDumpFirstError.callerPid, gCmdqDumpFirstError.callerName);
...
#ifdef CMDQ_INSTRUCTION_COUNT
static void cmdq_core_fill_module_instruction_count_record(const uint32_t *pCommand, uint32_t *pModuleCount)
{
...
static void cmdq_core_fill_task_instruction_count_record(RecordStruct *pRecord, const TaskStruct *pTask)
{
#ifdef CMDQ_INSTRUCTION_COUNT
	if (gCmdqContext.instructionCountLevel < 1) {
		return;
	...
	do {
		...
		switch (op) {
			case CMDQ_CODE_MOVE:
				if (1 & (pCommand[1] >> 23)) {
					...
					if (CMDQ_CODE_WRITE == op) {
						...
						cmdq_core_fill_module_instruction_count_record(pCommand, pRecord->writewmaskModule);
					}
					else if (CMDQ_CODE_POLL == op) {
						pRecord->instructionCount[CMDQ_INSTRUCTION_COUNT_POLLING]++;
						cmdq_core_fill_module_instruction_count_record(pCommand, pRecord->pollModule);
					}
					else {
			...
			case CMDQ_CODE_READ:
				...
				cmdq_core_fill_module_instruction_count_record(pCommand, pRecord->readModlule);
				break;
			case CMDQ_CODE_WRITE:
				pRecord->instructionCount[CMDQ_INSTRUCTION_COUNT_WRITE]++;
				cmdq_core_fill_module_instruction_count_record(pCommand, pRecord->writeModule);
				break;
			case CMDQ_CODE_WFE:
				       ...
				       uint32_t thread)
{
	...
	if (pRecord && pTask) {
		...
		cmdq_core_fill_task_instruction_count_record(pRecord, pTask);
	}
}
