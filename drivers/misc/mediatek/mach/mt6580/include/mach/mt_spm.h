...
#else  /* no device tree: 6795: 163+32=195, 6735: 165+32=197, 6580: 88+32=120 */
...
#define SPM_VDE_PWR_CON			(SPM_BASE + 0x210)//Rainier not support, not in CODA
...
#define SPM_MJC_PWR_CON			(SPM_BASE + 0x298)//Rainier, not in CODA
#define SPM_AUDIO_PWR_CON		(SPM_BASE + 0x29c)//Rainier, not in CODA
#define SPM_CA15_CPU0_PWR_CON		(SPM_BASE + 0x2a0)//Rainier not support, not in CODA
#define SPM_CA15_CPU1_PWR_CON		(SPM_BASE + 0x2a4)//Rainier not support, not in CODA
#define SPM_CA15_CPU2_PWR_CON		(SPM_BASE + 0x2a8)//Rainier not support, not in CODA
#define SPM_CA15_CPU3_PWR_CON		(SPM_BASE + 0x2ac)//Rainier not support, not in CODA
#define SPM_CA15_CPUTOP_PWR_CON		(SPM_BASE + 0x2b0)//Rainier not support, not in CODA
#define SPM_CA15_L1_PWR_CON		(SPM_BASE + 0x2b4)//Rainier not support, not in CODA
#define SPM_CA15_L2_PWR_CON		(SPM_BASE + 0x2b8)//Rainier not support, not in CODA
#define SPM_MFG_2D_PWR_CON      (SPM_BASE + 0x2c0) // Need to remove in K2//Rainier, not in CODA
#define SPM_MFG_ASYNC_PWR_CON		(SPM_BASE + 0x2c4)//mt6735//Rainier not support, not in CODA
#define SPM_MD32_SRAM_CON		(SPM_BASE + 0x2c8)//Rainier, not in CODA
...
#define SPM_SLEEP_DUAL_VCORE_PWR_CON	(SPM_BASE + 0x404)//Rainier, not in CODA
#define SPM_SLEEP_PTPOD2_CON		(SPM_BASE + 0x408)//Rainier, not in CODA
...
#define SPM_RF_CLK_CFG		        (SPM_BASE + 0x620)//mt6580
#define SPM_RF_CFG_SET		      (SPM_BASE + 0x624)//mt6580
#define SPM_RF_CLK_CFG_CLR		    (SPM_BASE + 0x628)//mt6580
...
#define SPM_SLEEP_MD32_WAKEUP_EVENT_MASK	(SPM_BASE + 0x818)//Rainier, not in CODA
#define SPM_PCM_WDT_TIMER_VAL		(SPM_BASE + 0x824)
#define SPM_PCM_WDT_TIMER_OUT		(SPM_BASE + 0x828)
#define SPM_PCM_MD32_MAILBOX		(SPM_BASE + 0x830)//Rainier, not in CODA
#define SPM_PCM_MD32_IRQ		(SPM_BASE + 0x834)//Rainier, not in CODA
...
#define SPM_SLEEP_MD32_ISR_RAW_STA	(SPM_BASE + 0x914)//Rainier, not in CODA
...
#define SPM_PCM_WDT_LATCH     (SPM_BASE+0xb24) //mt6580 //TODO: check Rainier new feature
...
#define SPM_CA15_CPU0_IRQ_MASK		(SPM_BASE + 0xb40)//Rainier, not in CODA
#define SPM_CA15_CPU1_IRQ_MASK		(SPM_BASE + 0xb44)//Rainier, not in CODA
#define SPM_CA15_CPU2_IRQ_MASK		(SPM_BASE + 0xb48)//Rainier, not in CODA
#define SPM_CA15_CPU3_IRQ_MASK		(SPM_BASE + 0xb4c)//Rainier, not in CODA
...
#define SPM_SLEEP_CA15_WFI0_EN		(SPM_BASE + 0xf10)//Rainier not support, not in CODA
#define SPM_SLEEP_CA15_WFI1_EN		(SPM_BASE + 0xf14)//Rainier not support, not in CODA
#define SPM_SLEEP_CA15_WFI2_EN		(SPM_BASE + 0xf18)//Rainier not support, not in CODA
#define SPM_SLEEP_CA15_WFI3_EN		(SPM_BASE + 0xf1c)//Rainier not support, not in CODA
...
//FIXME: double check with DE for PCM_FlAGS
...
#define SPM_EXT_VSEL_GPIO103    (1U << 11)//mt6735
...
//TODO: Consider 0xA0000000 start address ?
#else
#define SPM_CPU_PDN_DIS		(1U << 0)
#define SPM_INFRA_PDN_DIS	(1U << 1)
#define SPM_DDRPHY_PDN_DIS	(1U << 2)
...
#define SPM_WAKE_SRC_LIST	{	\
	...
	SPM_WAKE_SRC(1, SPM_IRQ1), /* mt6580 */	\
	SPM_WAKE_SRC(2, KP),		\
	SPM_WAKE_SRC(3, WDT),		\
	SPM_WAKE_SRC(4, GPT),		\
	SPM_WAKE_SRC(5, EINT),	\
	SPM_WAKE_SRC(6, CONN_WDT),		\
	SPM_WAKE_SRC(7, GCE), /* mt6580 */	\
	SPM_WAKE_SRC(8, CCIF0_MD),	/* mt6580 */ \
	...
	SPM_WAKE_SRC(16, MD1_VRF18_WAKE),	/* mt6580 */ \
	SPM_WAKE_SRC(17, MD1_VRF18_SLEEP), /* mt6580 */ \
	SPM_WAKE_SRC(18, DBGSYS), /* mt6580 */	\
	SPM_WAKE_SRC(19, UART0),	\
	SPM_WAKE_SRC(20, AFE),		\
	SPM_WAKE_SRC(21, THERM),	\
	SPM_WAKE_SRC(22, CIRQ),		\
	SPM_WAKE_SRC(23, SEJ), /* mt6580 */	\
	SPM_WAKE_SRC(24, SYSPWREQ),	\
	SPM_WAKE_SRC(25, MD1_WDT),	/* mt6580 */ \
	SPM_WAKE_SRC(26, CPU0_IRQ),	/* mt6580 */ \
	SPM_WAKE_SRC(27, CPU1_IRQ),	/* mt6580 */	\
	SPM_WAKE_SRC(28, CPU2_IRQ),	/* mt6580 */ \
	SPM_WAKE_SRC(29, CPU3_IRQ),	/* mt6580 */ \
