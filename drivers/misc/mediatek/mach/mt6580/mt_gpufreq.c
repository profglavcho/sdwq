 ...
 ****************************/
// TODO: enable this if spower api is ready
...
static void _mt_gpufreq_set_cur_freq(unsigned int freq_new)
{
#if 0   // TODO: waiting for clkmgr API
    int clksrc = 0;
    ...
    clkmux_sel(MT_CLKMUX_MFG_MUX_SEL, clksrc, "GPU_DVFS");
...
static unsigned int _mt_gpufreq_get_cur_freq(void)
{
#if 1   // TODO: waiting for clkmgr API
    return 0;
#else
    unsigned int freq = 0;
...
static void _mt_setup_gpufreqs_power_table(int num)
{
    ...
    for (i = 0; i < num; i++) {
        _mt_gpufreq_power_calculation(i, mt_gpufreqs_power[i].gpufreq_khz, mt_gpufreqs_power[i].gpufreq_volt, temp);
...
{
    int i = 0;
    unsigned int cur_vcore = _mt_gpufreq_get_cur_volt();

    gpufreq_info("@%s: cur_vcore = %d mV\n", __func__, cur_vcore / 100);
    ...
    for (i = 0; i < num; i++) {
        ...
        mt_gpufreqs[i].gpufreq_volt = cur_vcore;
