...
//extern u32 get_devinfo_with_index(u32 index); wait for maoyuan
u32 get_devinfo_with_index(u32 index)
{
	return 0;
}

#undef CONFIG_HAS_EARLYSUSPEND //wait for earlysuspend
...
#define CPU_DVFS_FREQ5   (903000) /* KHz */ //1.807/2
#define CPU_DVFS_FREQ6   (754000) /* KHz */ //1.508/2
#define CPU_DVFS_FREQ7   (604000) /* KHz */ //1.209/2
...
static struct mt_cpu_freq_info opp_tbl_e1_0[] = {
	OP(CPU_DVFS_FREQ1,  131250),
	OP(CPU_DVFS_FREQ2,  127500),
...
static struct mt_cpu_freq_info opp_tbl_e1_1[] = {
	...
	OP(CPU_DVFS_FREQ1,   131000),
	OP(CPU_DVFS_FREQ2,   127000),
	OP(CPU_DVFS_FREQ3,   120000),
	OP(CPU_DVFS_FREQ4,   115000),
...
#if 0
#define PLL_DIV1_1807_FREQ		(1807000) //for 900MHz
#define PLL_DIV1_1508_FREQ		(1508000) //for 1.5G & 750MHz
#define PLL_DIV1_1209_FREQ		(1209000) //for 1.2G & 600MHz
#define PLL_DIV1_1001_FREQ		(1001000) //for 1G - low
...
#define DDS_DIV1_1001_FREQ		(0x0009A000)	/* 1001MHz */
	...
	if (con1 >= DDS_DIV1_1001_FREQ) {
		freq = DDS_DIV1_1001_FREQ;
...
#define DDS_DIV1_1001_FREQ		(0x0009A000)	/* 1001MHz */
	...
	if (khz >= PLL_DIV1_1001_FREQ)
		dds = DDS_DIV1_1001_FREQ + ((khz - PLL_DIV1_1001_FREQ) * 2 / PLL_FREQ_STEP) * 0x2000;
    else
        BUG();
...
#define IS_CLKDIV_USED(clkdiv)  (((clkdiv < 8) || ((clkdiv % 8) == 0)) ? 0 : 1)
	...
	switch (target_khz) {
		...
		case CPU_DVFS_FREQ5:
			dds = _cpu_dds_calc(PLL_DIV1_1807_FREQ);  // 900= 1807 / 2
			sel = 10;    // 2/4
			break;
			
		case CPU_DVFS_FREQ6:
			dds = _cpu_dds_calc(PLL_DIV1_1508_FREQ);   // 750= 1495 / 2
			sel = 10;    // 2/4
			break;
		
		case CPU_DVFS_FREQ7:
			dds = _cpu_dds_calc(PLL_DIV1_1209_FREQ);   // 600 = 1209 / 2
			sel = 10;    // 2/4
			break;
	...
	// down but frequency may be higher, so clkdiv it first 
	if (target_khz < cur_khz)
		...
		cpufreq_write(ARMPLL_CON1, dds); //0x10018000	APMIXEDSYS + 0x0104
...
static unsigned int get_cur_volt_pmic_wrap(struct mt_cpu_dvfs *p)
{
    ...
    if (rdata) { /* enabled i.e. not 0mv */
        do {
			pwrap_read(PMIC_ADDR_VCORE_VOSEL_ON, &rdata); //wait for WY
...
#if 0  // Do NOT disable DVFS in early-suspend for LTE throughput issue
#ifdef CONFIG_CPU_FREQ
        policy = cpufreq_cpu_get(p->cpu_id);
...
{
	cpufreq_write((1 << RG_FQMTR_RST_BIT), FREQ_MTR_CTRL);
	dsb();
	cpufreq_write(0, FREQ_MTR_CTRL);
}
