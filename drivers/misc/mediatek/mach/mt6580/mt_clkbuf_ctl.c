...
static void clk_buf_ctrl_by_reg(CLK_BUF_SWCTRL_STATUS_T *status)
{
    ...
    {
        clk_buf_dbg("CLKBUF[%d] %d", i + 1, status[i]);
    }
    ...
    {
        mt_reg_sync_writel(CLK_BUF_CONN_SEL, RF_CK_BUF_REG_CLR);
        mt_reg_sync_writel(CLK_BUF_CONN_GRP_SET(1), RF_CK_BUF_REG_SET);
    }
    else if (status[1] == CLK_BUF_SW_ENABLE)
    {
        mt_reg_sync_writel(CLK_BUF_CONN_SEL, RF_CK_BUF_REG_SET);
        mt_reg_sync_writel(CLK_BUF_CONN_GRP_SET(1), RF_CK_BUF_REG_SET);
    }
    else // CLK_BUF_SW_CONNSRC
    {
        mt_reg_sync_writel(CLK_BUF_CONN_SEL, RF_CK_BUF_REG_CLR);
        mt_reg_sync_writel(CLK_BUF_CONN_GRP_SET(status[1]), RF_CK_BUF_REG_SET);
    }

    // CLK_BUF_NFC
    if (status[2] == CLK_BUF_SW_DISABLE)
    {
        mt_reg_sync_writel(CLK_BUF_NFC_SEL, RF_CK_BUF_REG_CLR);
        mt_reg_sync_writel(CLK_BUF_NFC_GRP_SET(1), RF_CK_BUF_REG_SET);
    }
    else if (status[2] == CLK_BUF_SW_ENABLE)
    {
        mt_reg_sync_writel(CLK_BUF_NFC_SEL, RF_CK_BUF_REG_SET);
        mt_reg_sync_writel(CLK_BUF_NFC_GRP_SET(1), RF_CK_BUF_REG_SET);
    }
    else // CLK_BUF_SW_NFCSRC or CLK_BUF_SW_NFC_NFCSRC
    {
        mt_reg_sync_writel(CLK_BUF_NFC_SEL, RF_CK_BUF_REG_CLR);
        mt_reg_sync_writel(CLK_BUF_NFC_GRP_SET(status[2]), RF_CK_BUF_REG_SET);
    }

    // CLK_BUF_AUDIO
    if (status[3] == CLK_BUF_SW_DISABLE)
    {
        mt_reg_sync_writel(CLK_BUF_AUDIO_SEL, RF_CK_BUF_REG_CLR);
        mt_reg_sync_writel(CLK_BUF_AUDIO_GRP_SET(1), RF_CK_BUF_REG_SET);
    }
    else // CLK_BUF_SW_ENABLE
    {
        mt_reg_sync_writel(CLK_BUF_AUDIO_SEL, RF_CK_BUF_REG_SET);
        mt_reg_sync_writel(CLK_BUF_AUDIO_GRP_SET(1), RF_CK_BUF_REG_SET);
    }
#endif
}
