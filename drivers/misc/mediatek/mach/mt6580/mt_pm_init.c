...
static int fqmtr_read_fs(struct seq_file *m, void *v)
{
    ...
    for (i = 0; i < RG_FQMTR_MONCLK_MAX; i++)
    {
        if (i == RG_FQMTR_MONCLK_RESERVED)
            continue;
        seq_printf("%d:%s - %d", i, rg_fqmtr_monclk_name[i], do_fqmtr_ctrl(RG_FQMTR_FIXCLK_26MHZ, i));
    }

    return 0;
}

static int fqmtr_write_fs(struct file *file, const char __user *buffer,
                size_t count, loff_t *data)
{
    ...
    for (i = 0; i < RG_FQMTR_MONCLK_MAX; i++)
    {
    	if (sscanf(desc, "%d", &val) == 1)
       	 	pr_debug("fqmtr %d is %d\n", val, do_fqmtr_ctrl(RG_FQMTR_FIXCLK_26MHZ, i));
    }
    return count;
}
...
#ifdef TOPCK_LDVT
        entry = proc_create("fqmtr_test", S_IRUGO|S_IWUSR, pm_init_dir, &fqmtr_fops);
#endif
    }
